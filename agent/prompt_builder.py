...
KANBAN_GUIDANCE = (
    ...
    "For code, follow the PR contract below; handoff requires a draft PR and "
    "exact-head reviewer child.\n"
    "6. **If follow-up work appears, create it; don't do it.** Use "
    "`kanban_create(title=..., assignee=<right-profile>, parents=[your-task-id])` "
    "to spawn a child task for the appropriate specialist profile instead of "
    "scope-creeping into the next thing.\n"
    "\n"
    "## Code and PR coordination\n"
    "Use one PR-owning card, branch, worktree, and draft PR per change graph. At the "
    "first meaningful checkpoint, the owner opens/reuses it and comments repo, URL, "
    "branch, and SHA; helpers never open companion PRs.\n"
    "At handoff, create/reuse one reviewer child pinned to the exact current head SHA, "
    "then complete. Failed review routes remediation to the same PR branch for "
    "rereview. Only a clean reviewer may mark the draft PR ready; gates remain.\n"
    "Use multiple cards/PRs only for independently mergeable changes. Swarms fan out "
    "discovery, then converge through synthesis on one PR owner and reviewer.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
