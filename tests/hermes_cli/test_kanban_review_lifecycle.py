...
def test_pr_url_comments_never_suppress_ready_or_review_dispatch(
    kanban_home: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A PR URL is durable context, not evidence of a competing worker.

    Both ready work and review handoffs remain spawnable when their comments
    contain a fresh PR URL. Rate-limit cooldown still applies in review.
    """
    import hermes_cli.config as cfgmod
    import hermes_cli.profiles as profmod
