...
The dispatcher refuses to re-spawn a ready task when it hit a quota/auth/429 error on the previous run (`blocker_auth`), or completed a run successfully within the guard window (`recent_success`), or a recent task comment links to a GitHub PR (`active_pr`). This prevents repeat worker storms on the same bug or task while a human catches up. Identical consecutive guard events are coalesced: dispatch diagnostics still report the guarded task every tick, but the durable event log records the unchanged guard state once. See the `respawn_guarded` row in the [event reference](#event-reference).

### Exact-SHA review routing

When a dedicated reviewer task is a parent of a PR-bearing producer, reviewers should complete with structured metadata containing `verdict` (`CLEAN` or `NOT CLEAN`), the GitHub `pr_url`, and a full 40-character `reviewed_sha` (the aliases `reviewed_pr`, `review_pr`, `head_sha`, and `exact_sha` are also accepted).

- `NOT CLEAN` creates or reuses exactly one remediation owner for that producer and reviewed SHA, plus one dependent re-review. The remediation card is constrained to the same branch and same PR; the re-review reads the remediation handoff's `replacement_sha` and reviews that immutable SHA. The re-review remains an unfinished parent of the original producer, so dependency promotion cannot feed the producer into the `active_pr` guard.
- `CLEAN` closes the original producer and promotes its downstream dependents normally.
- Missing or mismatched review evidence fails safe: the PR-bearing producer stays in `todo` and the reviewer records `review_routing_deferred` instead of creating guessed work or promoting a card that cannot run.

Remediation completion metadata must include `pr_url`, `branch`, and a full `replacement_sha`. Re-review completion uses the same review metadata contract. Each replacement SHA can therefore produce at most one further remediation/re-review pair, and no companion PR is created by the board router.
...
| `review_remediation_routed` | `{review_task_id, remediation_task_id, rereview_task_id, pr_url, reviewed_sha}` | A structured `NOT CLEAN` review created/reused the same-PR remediation and dependent exact-SHA re-review, then linked the re-review as a producer parent. |
| `review_approved` | `{review_task_id, pr_url, reviewed_sha, verdict}` | A structured `CLEAN` exact-SHA review closed the original PR producer. A normal `completed` event follows on that producer. |
| `review_routing_deferred` | `{verdict, reason}` | A PR-bearing review could not be routed safely because exact review evidence was missing/mismatched or routing failed. The producer remains in `todo`. |
